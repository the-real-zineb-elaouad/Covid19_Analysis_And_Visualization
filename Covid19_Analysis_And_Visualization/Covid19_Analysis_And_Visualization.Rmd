...
```{r}
# Sans transformation
g2<-ggplot(data_dual, aes(x = date)) +
  geom_line(aes(y = new_cases_smoothed, color = "Nouveaux cas (7j)"), size = 1) +
  geom_line(aes(y = new_deaths_smoothed, color = "Décès (7j)"), size = 1) +  # Pas de *20
  ...
  scale_y_continuous(
    name = "Valeurs brutes (moyenne 7j)",
    labels = scales::comma
  ) +
  
  scale_color_manual(
    values = c("Nouveaux cas (7j)" = "steelblue", "Décès (7j)" = "firebrick")
  ) +
  
  labs(
    title = "Comparaison sans transformation des cas et décès COVID-19",
    x = "Date",
    color = "Indicateur"
  ) +
