...
# Le graphe représentant le nombre moyen par jour de cas de décés enregisté pour les 5 pays les plus touchés
...
# Graphique pour les 5 Europe les plus touchés
...
p1 <- ggplot(data_top5_contamined_countries, aes(x = date, y = new_cases_smoothed, color = location)) +
  ...
  # TITRES ET LIBELLÉS
  labs(
    title = "Nombre moyens par jour de nouveaux cas enregisté pour les 5  pays les plus touchés d'Europe",
    x = "Date",
...
p1 <- ggplot(data_top5_contamined_countries, aes(x = date, y = new_deaths_smoothed, color = location)) +
  ...
  # TITRES ET LIBELLÉS
  labs(
    title = "L'évolution du nombre moyens par jour de nouveaux décés enregisté pour les 5  pays les plus touchés d'Europe",
    x = "Date",
...
# On crée un nouveau data frame avec les deux variables dans des colonnes séparées
...
compare <- ggplot(data_dual, aes(x = date)) +
  geom_line(aes(y = new_cases_smoothed, color = "Nouveaux cas (7j)"), size = 1) +
  geom_line(aes(y = new_deaths_smoothed * 20, color = "Décès (7j) x20"), size = 1) +  # Multiplié pour rendre visible
  ...
  scale_color_manual(
    values = c("Nouveaux cas (7j)" = "steelblue", "Décès (7j) x20" = "firebrick")
  ) +
  
  labs(
    title = "Comparaison des nouveaux cas et des décès COVID-19 (moyenne 7j)",
    x = "Date",
    color = "Indicateur"
  ) +
