...
```{r}
df_evolution_temporelle %>% mutate(
    reproduction_rate_7d_avg = sapply(1:nrow(df_evolution_temporelle), function(i) {
      if (i < 7) {
        return(NA)
      } else {
        return(mean(df_evolution_temporelle$reproduction_rate[(i-6):i], na.rm = TRUE))
      }
    }),
    
    stringency_index_7d_avg = sapply(1:nrow(df_evolution_temporelle), function(i) {
      if (i < 7) {
        return(NA)
      } else {
        return(mean(df_evolution_temporelle$stringency_index[(i-6):i], na.rm = TRUE))
      }
    })
  )
