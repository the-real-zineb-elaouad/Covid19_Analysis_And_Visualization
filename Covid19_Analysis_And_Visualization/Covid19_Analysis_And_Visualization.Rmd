...
## -----------------------------------------------------------------  ZINEB -------------------------------------------------------------
```{r}
#question :Quelle est la relation entre le nombre de cas et le nombre de décès (taux de létalité) ?
#question : Relation entre nombre de cas et décès (taux de létalité)
library(ggplot2)
library(dplyr)
library(lubridate)

# Solution 1 : Utilisation du package here pour les chemins relatifs
covid_data <- read.csv("../data/owid-covid-data.csv", sep = ",") 
...
# Vérification des noms de continents
unique(covid_data$continent) # Ajuster les filtres selon les résultats

# Nettoyage des données avec vérifications
df_summary <- covid_data %>%
  filter(
    continent %in% c("Africa", "Asia", "Europe", "North America", "South America", "Oceania"),
    !is.na(total_cases),
    !is.na(total_deaths),
    total_cases > 0,
    total_deaths > 0
...
# Visualisation avec gestion des valeurs extrêmes
ggplot(df_summary, aes(x = total_cases, y = total_deaths)) +
  geom_point(aes(color = continent), alpha = 0.6, size = 2.5) +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, color = "darkred") +
  scale_x_log10(labels = scales::label_number(scale_cut = scales::cut_short_scale())) +
  scale_y_log10(labels = scales::label_number(scale_cut = scales::cut_short_scale())) +
  labs(
    title = "Relation cas-décès COVID-19 par pays (2020-2022)",
    subtitle = "Données Our World in Data",
    x = "Cas confirmés (log10)",
    y = "Décès enregistrés (log10)",
    color = "Continent"
  ) +
